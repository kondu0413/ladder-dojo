...
| 画面 | パス | 内容 |
|---|---|---|
| トップ | `/` | 未ログインなら紹介の画面(LP)、ログイン済みなら問題一覧。LP からは「ログインせずに試す」で問題に入れる(S-021)。LP の見本のラダー図はその場で押して動かせる |
| 問題一覧 | `/problems` | 公式問題 50 問を段階別に表示。段階ごとの進み具合とクリア状況、「続きはここから」、クリア状況 × モードの絞り込みつき。未ログインでも開ける |
| 問題 | `/problems/:id` | 読む(選択式の予測 + 解説 + シミュレータで答え合わせ)/ 直す / 書く。正解したら「次の問題へ」。問題文の用語は用語集へのリンク |
| みんなの問題 | `/community` | ユーザーが投稿した問題の一覧。検索・並べ替え・絞り込み |
| 投稿問題 | `/community/:id` | 投稿問題を解く。いいね・難易度投票・通報 |
| サンドボックス | `/sandbox` | 自由に回路を作り、テストを付けて保存・実行・投稿。「動かす」の操作を記録してそのままテストにできる。共有リンク(`#c=...`)で回路をログイン不要で渡せる |
| ランキング | `/rankings` | 指標 4 種 × 期間 3 種。速さは競わせない |
| 組織 | `/orgs` | 組織の作成・招待コードでの参加 |
| 組織の詳細 | `/orgs/:id` | メンバー / 課題 / 一覧表 / つまずき。課題には期限と達成率が付く。管理者はメンバー × 問題の表で全員の進捗を一望でき、CSV で保存もできる。課題の期限と新しい課題は、毎朝 9 時に Web Push で受け取れる(0 円、S-045) |
| サンプル | `/samples` | 代表的な回路 7 種を動かして見る(メニューから開ける) |
| 用語集 | `/glossary` | ラング・a 接点・自己保持・インターロック・SET/RST・オフディレイなど 25 語。語ごとに小さな回路を置いて動かせる |
...
- タイマは実時間。1x / 5x / 即時 を切り替えられる。一時停止中は「1 スキャン」ずつ進められる
- 命令は a / b 接点・立ち上がり・立ち下がり接点、出力・SET・PLS・オンディレイ・オフディレイ・カウンタ・RST(S-044)
- 回路エディタは元に戻す / やり直す(Ctrl+Z / Ctrl+Shift+Z)が効く
- ダークモードに対応。端末の設定に従い、ヘッダーのボタンで切り替えられる(S-043)
- 電流が流れている線には流れの印が動く。動きを減らす設定の端末では出さない
...
- 見た目の部品は `apps/web/src/components/ui.tsx` に集めてある(S-036)。ボタン・切り替え・入力欄・お知らせ・アイコンはここから使う。色は紺(ヘッダー・母線・主役のボタン)と橙(通電・答え合わせ・進捗)の 2 軸で、緑 / 赤は正解 / 不正解にだけ使う。Web フォントや UI ライブラリは入れていない
- ログイン状態はヘッダーの右端に出る。端末に溜めた進捗の引き継ぎと、同期に失敗したときの案内は本文の先頭に出る
...
E2E は `apps/api` の `e2e:serve` が `wrangler dev --env e2e` を起動する。`E2E_BASE_URL` を設定すると既存サーバーに接続する。手元の Chromium を使いたいときは `PLAYWRIGHT_CHROMIUM_PATH=/path/to/chrome`(Claude Code の環境では `/opt/pw-browsers/chromium`)。

E2E は「使えるか」だけでなく、ログイン後に進捗の取得が繰り返されないこと(回数)も見ている(S-037)。
