...
- **却下**:
  ...
  - Cloudflare D1 + Better Auth: **最有力の代替案**。1ベンダーで完結し、容量 5 GB・一時停止なし・Time Travel による 30 日復元と、運用面では Supabase 無料枠より楽。却下理由は能力不足ではなく**自前実装が増える**こと: 認証(セッション・メール確認)を Workers 上で運用し、RLS がないため保存・取得・組織権限判定を全て Worker の API として書き、テストする必要がある。加えて全 API が Workers 無料枠(10 万リクエスト/日)と D1 の書き込み上限(10 万行/日)に乗る。Supabase の 7 日停止・500 MB が運用上の負担になった場合の乗り換え先として、この構成(Pages + Workers/Hono + D1 + Drizzle + Better Auth)を第一候補とする(2026-09-05 人間確認済み・Supabase 維持で確定)
