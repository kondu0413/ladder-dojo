...
## 現在のフェーズ: フェーズ2(みんなで増やす)— 投稿問題の API と画面を実装中
...
### 次にやること(Claude Code)
1. **人間にお願い**: 本番 URL で Google ログインを一度試して、実際にログインできることを確認してほしい(E2E ではメール/パスワードの代替ログインしか通せないため)
2. フェーズ2 の残り: ドキュメント整備を終えて DoD を確認する
3. フェーズ3(§3.7 ランキング、§3.8 管理者ビュー)に進む
