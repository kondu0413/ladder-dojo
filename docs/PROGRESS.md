...
## 現在のフェーズ: フェーズ1(一人で学べる)— 学習モードと公式問題まで完了。残りはログイン UI と進捗同期
...
### 動作確認(この環境で実行、2026-09-15 学習モード 時点)
...
### 次にやること(Claude Code)
1. Google ログイン UI と進捗同期(localStorage → サーバーのマージ、S-002)
2. サンドボックスの保存とテストケース編集(§3.4)
3. フェーズ1 DoD を満たしたらフェーズ2 へ
