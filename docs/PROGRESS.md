...
## 現在のフェーズ: フェーズ1(一人で学べる)— 回路の閲覧・操作・編集まで完了
...
### 動作確認(この環境で実行、2026-09-15 回路の編集 時点)
...
### 次にやること(Claude Code)
1. 学習モードの画面(読む / 直す / 書く)と判定結果の表示(不正解時の差分、模範解答と指標の比較)
2. 公式問題 30 問以上(自己保持 / タイマ / カウンタ / インターロック / 組み合わせ × 各モード 2 問以上)
3. Google ログイン UI、進捗同期、サンドボックスの保存とテストケース編集
4. フェーズ1 DoD を満たしたらフェーズ2 へ
